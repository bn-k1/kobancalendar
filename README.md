...
- `data` を配列にすると、**基準日（＝回転の起点）を動かさずに、世代の途中である日からCSVフォルダだけを差し替え**られます（コマ位置・回転はそのまま、表示の「新版/旧版」切替も増えません）。詳細は下記「運用: スケジュールの移行」を参照。
...
- 全セグメントは**同じサイクル長**（CSVの行数）でなければなりません（回転位相を維持するため。違反はビルドエラー）。差し替えるのはあくまで「表の中身」で、回転の長さやコマ位置の意味は変えない用途です。
- セグメントの `from` は、その世代の表示窓（次の世代の `from` まで）の中に収めてください。
- この差し替えはカレンダー表示だけでなく「飲みに行くンダー」の予定調整にも反映されます。
- 回転位相そのものを動かしたい（コマ位置を一斉にずらす）場合は、配列ではなく**別世代**（別の `from`）として追記してください（上記2）。
