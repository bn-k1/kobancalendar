...
export class MeetupFinderService {
  ...
  canAttendMeetup(schedule, meetupStartTime) {
    ...
    const endTimeObj = dayjs().hour(parseInt(endHour, 10)).minute(parseInt(endMinute, 10));
    
    // 飲み会開始時間を解析
    const [meetupHour, meetupMinute = "00"] = meetupStartTime.split(":");
    const meetupTimeObj = dayjs().hour(parseInt(meetupHour, 10)).minute(parseInt(meetupMinute, 10));
   ...
   * @returns {Object} 確認結果
   */
  checkDateForPositions(date, positions, meetupStartTime, currentBaseDate, lastBaseDate) {
    const result = {
      availablePositions: [],
      details: []
    };
