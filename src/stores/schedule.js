...
export const useScheduleStore = defineStore('schedule', () => {
  ...
  function calculateShiftIndex(targetDate, startPosition, baseDate) {
    ...
    const shiftIndex = 
      (((adjustedStartPosition + daysDifference) % cycleLength) + cycleLength) % cycleLength;
  ...
  // 日付範囲のスケジュールを計算
  function calculateScheduleRange(startDate, endDate, startPosition, baseDateParam) {
    const scheduleRange = [];
  ...
  function processCSVData(csvData) {
    console.log('CSV処理前データ:', typeof csvData, csvData?.substring ? csvData.substring(0, 100) : csvData);
