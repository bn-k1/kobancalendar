...
KobanCalendar を開発・改造する人、および `config/`・`data/` の生ファイルを直接編集する人のためのリファレンスです。アプリの導入と日々の運用にこのドキュメントは不要です（[README](README.md) と管理ガイドを参照してください）。
...
`main` に push すると `.github/workflows/deploy.yml` が走り、`build-prep` → テスト → `build-gh-pages` を実行して GitHub Pages へ配信します。Pages の Source 設定（初回のみ）は [README](README.md) の導入手順にあります。
